...
Then (full pass even if all images skipped):
  ...
  Update centroids
  Write clusters/latest.json, identities.json, index.json
...
def _meta_dir() -> Path:
    """Absolute path to export/metadata."""
    return Path(config["output_path"]) / "metadata"


def _face_meta_dir() -> Path:
    return _meta_dir() / "faces"


def _detection_path(gallery_id: str, image_id: str) -> Path:
    return _face_meta_dir() / FACES_DETECTIONS_DIR / gallery_id / f"{image_id}.json"


def _embedding_path(face_id: str) -> Path:
    return _face_meta_dir() / FACES_EMBEDDINGS_DIR / f"{face_id}.bin"


def _crop_path(gallery_id: str, image_stem: str, face_index: int) -> Path:
    return _face_meta_dir() / FACES_CROPS_DIR / gallery_id / f"{image_stem}_{face_index}.jpg"
...
def _write_index_json(
    ...
    out = _face_meta_dir() / FACES_INDEX_JSON
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w") as fh:
        json.dump(data, fh, indent=2)


def _write_identities_json(
    store: IdentityStore,
    all_detections: dict[str, dict[str, Any]],
    ...
    result: dict[str, Any] = {}
    for slug, entry in store.identities.items():
        exemplar_face_ids: list[str] = []
        positive_count = 0
        propagated_count = 0

        # Collect exemplar face IDs from detection records
        for det in all_detections.values():
            for face in det["faces"]:
                if face.get("identity_id") == slug:
                    if face["provenance"] == FACE_PROVENANCE_POSITIVE:
                        exemplar_face_ids.append(face["face_id"])
                        positive_count += 1
                    elif face["provenance"] == "propagated":
                        propagated_count += 1
    ...
    out = _face_meta_dir() / FACES_IDENTITIES_JSON
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w") as fh:
...
def _write_centroids(centroids: dict[str, np.ndarray]) -> None:
    emb_dir = _face_meta_dir() / FACES_EMBEDDINGS_DIR
...
def _write_clusters_json(all_detections: dict[str, dict[str, Any]]) -> None:
    ...
    out = _face_meta_dir() / FACES_CLUSTERS_LATEST_JSON
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w") as fh:
...
def run() -> FaceStageResult:
    ...
    gallery_names = sorted(
        g for g in os.listdir(source) if os.path.isdir(os.path.join(source, g))
    )
    ...
    with create_file_progress(console) as progress:
        ...
        _write_identities_json(store, all_detections, centroids)
        _write_clusters_json(all_detections)

        # ── Phase 9: sync auto-tags ─────────────────────────────────────────
...
def recluster() -> int:
    ...
    all_detections: dict[str, dict] = {}
    for gallery_id in gallery_names:
        det_dir = _face_meta_dir() / FACES_DETECTIONS_DIR / gallery_id
